...
static void * request_thread(void *threadArgs) {
   ...
   while(1) {
      ...
      if(status == sizeof(packet)) {
         ...
         if(g_verbose) {
            printf("%3d,  RX, %s, %d, 0x%08x\n", info->current_position, s, packet.delay, packet.uuid[0]);
         }
         ...
         if(packet.delay > 0 && packet.delay < MAX_DELAY_SECONDS) {
...
int main(int argc, char * argv[])
{
   ...
   if(argc > 1) {
      if(strcmp(argv[1], "-v") == 0) {
         ...
         printf("%s: Usage -v = Verbose output\n");
         exit(-1);
      }
   }
   ...
   while(1) {
         ...
         if(status > 0) {
            ...
            for(cmsg = CMSG_FIRSTHDR(&msg); cmsg != NULL; cmsg = CMSG_NXTHDR(&msg, cmsg)) {
               if(cmsg->cmsg_level == IPPROTO_IPV6 && cmsg->cmsg_type == IPV6_RECVERR)
               {
                  if(g_verbose) {
                       char s[256] = {0,};
                     inet_ntop(AF_INET6, &remote, s, sizeof(s)-1);
                     printf("%3d, ERR, %s\n", request_info.current_position, s);
                  }
               }
            }
         }
      ...
      while(!SLIST_EMPTY(&request_info.ping_table[request_info.current_position])) {
         ...
         if(g_verbose) {
            char s[256] = {0,};
            inet_ntop(AF_INET6, &entry->addr, s, sizeof(s)-1);
            printf("%3d,  TX, %s, %d, 0x%08x\n", request_info.current_position, s, packet.delay, packet.uuid[0]);
         }
         // Reply to device
